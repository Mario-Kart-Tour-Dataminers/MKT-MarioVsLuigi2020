    ...
    Mtl_FragmentIn input [[ stage_in ]])
{
    ...
    u_xlat0.w = (-float(u_xlat16_0)) + 1.0;
    ...
    Mtl_FragmentIn input [[ stage_in ]])
{
    ...
    u_xlat0.xyz = fma(float3(u_xlat11), float3(u_xlat16_2.xyz), float3(FGlobals.unity_FogColor.xyz));
    output.SV_Target0 = half4(u_xlat0);
    return output;
}
"
    ...
    Mtl_FragmentIn input [[ stage_in ]])
{
    ...
    if(((int(u_xlatb2) * int(0xffffffffu)))!=0){discard_fragment();}
    u_xlat16_2.xyz = _MainTex.sample(sampler_MainTex, input.TEXCOORD0.xy).xyz;
    u_xlat0.xyz = float3(u_xlat16_2.xyz) * input.COLOR0.xyz;
    output.SV_Target0 = half4(u_xlat0);
    return output;
}
"
    ...
    Mtl_FragmentIn input [[ stage_in ]])
{
    ...
    if(((int(u_xlatb2) * int(0xffffffffu)))!=0){discard_fragment();}
    u_xlat16_2.xyz = _MainTex.sample(sampler_MainTex, input.TEXCOORD0.xy).xyz;
    u_xlat2.xyz = fma(float3(u_xlat16_2.xyz), input.COLOR0.xyz, (-float3(FGlobals.unity_FogColor.xyz)));
    u_xlat11 = input.TEXCOORD1;
    u_xlat11 = clamp(u_xlat11, 0.0f, 1.0f);
